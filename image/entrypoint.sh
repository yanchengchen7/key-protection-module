...
main() {
  ...
  echo "KPS: entrypoint started" > /dev/ttyS0 2>/dev/null || true

  # 1. IMA Policy Loading
  if [[ ! -f /usr/share/oem/ima-policy ]]; then
    if is_debug_mode; then
      echo "CRITICAL ERROR KPS: required IMA policy is missing in debug mode!" > /dev/ttyS0 2>/dev/null || true
    else
      echo "KPS: required IMA policy is missing" > /dev/ttyS0 2>/dev/null || true
      return 1 # Fail closed
    fi
  else
    if ! cp /usr/share/oem/ima-policy /sys/kernel/security/ima/policy; then
      if is_debug_mode; then
        echo "CRITICAL ERROR KPS: failed to load required IMA policy in debug mode!" > /dev/ttyS0 2>/dev/null || true
      else
        echo "KPS: failed to load required IMA policy" > /dev/ttyS0 2>/dev/null || true
        return 1 # Fail closed
      fi
    fi
  fi
  ...
  if ! /usr/share/oem/kps/network_setup.sh; then
    echo "KPS: network setup failed" > /dev/ttyS0 2>/dev/null || true
  fi
